...
const CreditsSliderContainer = ({ organisation, handleModalOpening }) => {
  ...
  const handleSliderOpening = operator => {
    ...
    if (operator.log && organisation.id)
      dispatch(
        getLogsByOrganisationRequest({ organisationId: organisation.id })
      )

    // Opens the editor for add/edit functionality
    if (operator.add || operator.edit)
      dispatch(addCredits(operator.add ? 0 : organisation.credits))
...
export default CreditsSliderContainer
