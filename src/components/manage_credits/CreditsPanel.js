...
import { addCredits, closeModal, triggerSlider } from "../../redux/actions/companies.action";
// Styles
...
const CreditsPanel = () => {
  ...
  const handleSliderOpening = (operator) => {
    if (operator.add || operator.edit) dispatch(addCredits(operator.add ? 0 : companyObj.credits));
